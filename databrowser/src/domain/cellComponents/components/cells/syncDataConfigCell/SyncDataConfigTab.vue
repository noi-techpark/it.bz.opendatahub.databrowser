...
<template>
  <EditListTab :items="items">
    <template #tabLabel="{ index }">
      <div class="w-full">SyncDataConfig {{ index + 1 }}</div>
    </template>
    ...
    >
      <div class="flex flex-wrap gap-8 md:flex-nowrap">
        <div class="basis-full md:order-1 md:basis-2/3">
          ...
          <SubCategoryItem title="SyncDataApiUrl">
            <StringCell :text="item.syncdataapiurl" :editable="false" />
          </SubCategoryItem>
        </div>
...
import { SyncDataConfigEntry } from './types';

defineProps<{ items: SyncDataConfigEntry[] }>();

const { addItem, deleteItems, duplicateItem, updateItem } =
  useInjectActionTriggers<SyncDataConfigEntry>();

const { editable } = useInjectEditMode();

const addPathparamItem = (index: number, item: SyncDataConfigEntry) => {
